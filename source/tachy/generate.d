...
private immutable string sampleTasks = q"[# A tachy tasks file (Pravic): every directive is an independent
# statement, jobs run in source order, and every job is an idempotent
# "ensure" — applying this file twice changes nothing the second time.
# Generated by "tachy generate task"; run it with:
#   tachy apply <selection> <this file>

var admin = "root"                # variables, lowest precedence here

directory /tmp/tachy-demo {       # state (default directory; absent),
    mode = "0755"                 # mode, owner, group
}

file /tmp/tachy-demo/hello.txt {
...
check "demo file exists" {        # a check: a command plus expectations
    run = "test -f /tmp/tachy-demo/hello.txt"
                                  # exit_status, output = "..." or
                                  # { contains = "..." } / { matches = "..." }
}

# Composition: an include composes another tasks file at its position,
# binding variables for it.
# include "base.pravic" {
#     env = "prod",
# }
]";

private immutable string sampleSettings = q"[# tachy settings — optional, read once at the start of every run.
...
imports {
    ...
    # relative to their defining tasks file, in order; the first
    # existing match wins.
    paths = [
        # "libs",
...
webui {
    ...
    # project entered through its main.pravic, a plain file is the entry
    # point itself.
    projects = [
        # "~/Code/site",
    ]
