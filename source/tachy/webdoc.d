 ...
 *
 * The HTTP layer is the webui's: `tachy.web`'s thread-per-connection
 * server and `:param` router, serving only GETs on localhost by default
 * (`--address`/`--port`).
 */
...
private enum string docSource = import("DOCUMENTATION.md");
...
private Section[] splitSections(string md, ref string[string] anchorPage)
{
...
private string inlineMd(string s, const(string[string])* anchors)
{
...
/// (settings.toml)" -> settings-settingstoml).
private string slugify(string t) @safe pure
{
...
private string compactOf(string id) @safe pure
{
...
private string menuHtml(in Section[] all, string current)
{
    auto app = appender!string;
...
private string layout(string title, string menu, string content)
{
    return "<!doctype html>\n<html>\n<head>\n<meta charset=\"utf-8\">\n<title>"
...
private enum string pageCss = import("tachy/webdoc/doc.css");
...
unittest // slugify and compact aliases match the file's link spellings
{
    ...
    assert(slugify("Settings (settings.toml)") == "settings-settingstoml");
    assert(compactOf("settings-settingstoml") == "settingssettingstoml");
...
unittest // inline markdown: escaping, code, bold, italic, links
{
    ...
    anchors["settingssettingstoml"] = "6-settings-settingstoml";
    ...
    assert(inlineMd("[s](#settingssettingstoml)", &anchors)
        == `<a href="/doc/6-settings-settingstoml#settingssettingstoml">s</a>`);
...
unittest // block rendering: fences, tables, lists, wrapped paragraphs
{
    ...
    {
        auto html = rend.render(splitLines("```toml\na = \"<b>\"\n```"));
        assert(html.canFind("<pre><code class=\"lang-toml\">a = &quot;&lt;b&gt;&quot;\n</code></pre>"),
            html);
    }
    {
