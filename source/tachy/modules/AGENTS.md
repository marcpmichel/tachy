...
- Directives → module names are wired in `models.d` (`addJobs`/
  `addHooks`): keys are targets, `path`/`name` is injected, both TOML
...
- Check mode: probes run, mutations go through `mustRun` (suppressed
  under `--check`); execute-style checks are checks by nature and run
  even in check mode, never reporting `changed`
