...
struct OutputExpectation {
    ...
    bool matches(string output) const
    {
        final switch(kind) {
            case Kind.exact:
                return output == text_;
            case Kind.contains:
                return indexOf(output, text_) >= 0;
            case Kind.matches:
                return !matchFirst(output, re).empty;
            case Kind.and_:
                ...
                return true;
            case Kind.not_:
                return !children[0].matches(output);
            case Kind.any_:
                foreach(const ref c; children)
                    if(c.matches(output))
                        return true;
    ...
    string describe() const @safe pure nothrow
    {
        ...
        final switch(kind) {
            case Kind.exact:
                return "exactly \"" ~ text_ ~ "\"";
            case Kind.contains:
                return "a substring \"" ~ text_ ~ "\"";
            case Kind.matches:
                return "a match of /" ~ pattern ~ "/";
            case Kind.and_:
                return children.map!(c => c.describe()).join(" and ");
            case Kind.not_:
                return "not (" ~ children[0].describe() ~ ")";
            case Kind.any_:
...
OutputExpectation parseOutput(in Val v, string context)
{
    ...
    if(v.kind != Val.Kind.table_)
        throw new TachyError(
                context ~ ": 'output' must be a string or a"
...
TaskResult runEnsureModule(Val[string] params, TaskContext ctx)
{
    ...
    if(!exitExp.matches(r.status))
        throw new TachyError("ensure '" ~ name ~ "': exit status " ~ text(r.status)
                ...
                ~ (got.length ? "; output: '" ~ excerpt(got) ~ "'" : errOut.length ? "; stderr: '" ~ excerpt(errOut) ~ "'" : ""));
...
/// the `probe` module's body assertions).
